...
// Read Design Guidelines Tool
server.tool(
  "read_design_guidelines",
  "Read design system guidelines and specifications from JSON files in the guides/guidelines folder",
  {
    filename: z.string().optional().describe("Optional specific filename to read (without .json extension). If not provided, lists all available files.")
  },
  async ({ filename }: any) => {
    try {
      ...
      if (!fs.existsSync(guidelinesDir)) {
        return {
          content: [
            {
              type: "text",
              text: "Guidelines directory not found. Please create the 'guides/guidelines' folder."
            }
          ]
        };
      }
      ...
      if (!filename) {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Available design guidelines:\n${files.map(f => `- ${f}`).join('\n')}\n\nUse read_design_guidelines with a filename to read a specific guideline.`
            }
          ]
        };
      }
      ...
      if (!fs.existsSync(filepath)) {
        return {
          content: [
            {
              type: "text",
              text: `Guideline file '${filename}.json' not found in guides/guidelines folder.`
            }
          ]
        };
      }
      ...
      return {
        content: [
          {
            type: "text",
            text: `Design Guideline: ${filename}\n\n${JSON.stringify(data, null, 2)}`
          }
        ]
      };
    } catch (error) {
      return {
        content: [
          {
            type: "text",
            text: `Error reading design guidelines: ${error instanceof Error ? error.message : String(error)}`
          }
        ]
      };
    }
...
// Get Analysis Prompt Tool
server.tool(
  "get_analysis_prompt",
  "Read analysis frameworks and checklists from JSON files in the guides/analysis-prompts folder",
  {
    filename: z.string().optional().describe("Optional specific filename to read (without .json extension). If not provided, lists all available prompts.")
  },
  async ({ filename }: any) => {
    try {
      ...
      if (!fs.existsSync(promptsDir)) {
        return {
          content: [
            {
              type: "text",
              text: "Analysis prompts directory not found. Please create the 'guides/analysis-prompts' folder."
            }
          ]
        };
      }
      ...
      if (!filename) {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Available analysis prompts:\n${files.map(f => `- ${f}`).join('\n')}\n\nUse get_analysis_prompt with a filename to read a specific prompt.`
            }
          ]
        };
      }
      ...
      if (!fs.existsSync(filepath)) {
        return {
          content: [
            {
              type: "text",
              text: `Analysis prompt file '${filename}.json' not found in guides/analysis-prompts folder.`
            }
          ]
        };
      }
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error reading analysis prompt: ${error instanceof Error ? error.message : String(error)}`
          }
        ]
      };
    }
